...
//De resto, a função é simples. Lê a imagem src, verfica se não há erros e depois precorre tudo até encontrar pixeis que estejam entre o threshold e threshold2 (100 e 180);
...
int vc_gray_to_bin(IVC *src, IVC *dst)
{
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			if (datasrc[pos] > threshold && datasrc[pos] < threshold2) //Cada posicao com intensidade acima da média fica a branco
			{
				datadst[pos] = 255;
			}
...
//Funcao que irá pegar na imagem original do crânio (ainda em tons de cinzento) e meter por cima dela outra imagem que é o resultado de vários tratamentos, já com o excessos retirados;
//Por sua vez, ira verficar quais os pixeis que estao a branco na segunda imagem e entao criar uma nova imagem com todos os pixeis da segunda imagem porem com o valor em tons de cinzento da primeira imagem; 
int vc_convert(IVC *src, IVC *src2, IVC *dst)
{
	...
	{
		printf("Erro: vc_convert()\n-Imagem vazia!\n");
		getchar();
		return 0;
	}

	if ((src->width <= 0) || (src->height <= 0) || (src->data == NULL) || (src2->width <= 0) || (src2->height <= 0) || (src2->data == NULL))
	{
		printf("Erro: vc_convert()\n-Dimensoes ou informacoes erradas!\n");
		getchar();
		return 0;
	...
	if ((src->channels != 1) || (src2->channels != 1))
	{
		printf("Erro: vc_convert()\n-Imagem com formato errado!\n");
		getchar();
		return 0;
	...
	for (y = 0; y < height; y++)
	{
		for (x = 0; x < width; x++)
		{
			...
			if (datasrc[pos] > 0  && datasrc2[pos] == 255) //Cada posicao com intensidade acima da média fica a branco
			{
